if object_id('mfr_plans_jdetail_calc_queue') is not null drop proc mfr_plans_jdetail_calc_queue
go
create proc mfr_plans_jdetail_calc_queue
	@detail_id int = null,
...
begin
	...
	declare @group_name varchar(30) = 'jobs-details-agent'
	...
	begin
		-- create new queue
		set @queue_id = newid()
		declare @sql_cmd nvarchar(max) = concat('exec mfr_plans_jdetail_calc @queue_id = ''', @queue_id, '''')
		-- push
		exec queue_append @queue_id = @queue_id, @thread_id = 'mfrs',
			@group_name = @group_name,
			@name = 'Агент сменных заданий',
			@sql_cmd = @sql_cmd,
			@priority = 0,
			@use_buffer = 0
		-- push objs
		if @detail_id is not null
			insert into queues_objs(queue_id, obj_type, obj_id) select @queue_id, 'mco', @detail_id
		else

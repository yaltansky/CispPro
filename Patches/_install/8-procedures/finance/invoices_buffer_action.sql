		...
		begin
			...
			-- шапки заявок
				...
				select 
					...
					'RUR',
					max(d.mol_id),
					concat('по документу ', d.invoice_number,  ' от ',  convert(varchar, inv.d_doc, 104)),
					concat('/dummy/ref/', d.agent_id)
				from @details d
                    join supply_invoices inv on inv.doc_id = d.invoice_id
				group by d.invoice_id, d.invoice_number, d.agent_id, inv.d_doc

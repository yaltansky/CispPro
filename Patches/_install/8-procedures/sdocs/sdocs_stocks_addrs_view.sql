if object_id('sdocs_stocks_addrs_view') is not null drop proc sdocs_stocks_addrs_view
go
-- exec sdocs_stocks_addrs_view 1000
-- @@STANDARD VIEW
create proc sdocs_stocks_addrs_view
	@mol_id int,	
	-- filter
    @stock_id int = null,
...
-- pattern params
	declare @pkey varchar(50) = 'ADDR_ID'
	declare @base_view varchar(50) = 'V_SDOCS_STOCKS_ADDRS'
	declare @obj_type varchar(3) = 'ADDR'
...
-- prepare sql
	...
	declare @where nvarchar(max) = concat(
        ...
        , case when @stock_id is not null then concat(' and (x.stock_id = ', @stock_id, ')') end

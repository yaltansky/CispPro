...
-- prepare sql
	...
	declare @where nvarchar(max) = concat(
		...
		, case when @search is not null then ' and (x.item_name like @search or x.note like @search)' end
        , case when @filter_attrs = 1 then concat(' and (x.item_id in (select product_id from ', @productsQueryTable, 
            ') or exists(select 1 from mfr_pdm_items where pdm_id = x.pdm_id and item_id in (select product_id from ', @productsQueryTable, ')))') end
		)

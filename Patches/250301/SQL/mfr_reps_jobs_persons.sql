    ...
    if @period_from is null begin
        exec tracer_log @tid, 'labor_hours'
            ...
            from (
                ...
                from mfr_plans_jobs_details jd
                    ...
                    left join mfr_plans_jobs_executors e on e.detail_id = jd.id
        ...
        exec tracer_log @tid, 'norm_labor_hours'
            ...
            where o.mfr_doc_id in (select id from @docs)
                and (@filter_jobs is null or o.oper_id in (select oper_id from #result))
                and (@filter_contents is null or c.content_id in (select id from #contents))
        ...
        -- labor_hours
            ...
            select 
                ...
                sum(c.q_brutto_product * do.duration_wk * dur1.factor / dur_h.factor),
                sum(case when o.status_id = 100 then o.duration_wk * dur2.factor / dur_h.factor end)
            from sdocs_mfr_opers o
                join sdocs_mfr_contents c on c.content_id = o.content_id
                    join sdocs_mfr_drafts_opers do on do.draft_id = c.draft_id and do.number = o.number
                        left join sdocs_mfr_drafts_opers_executors e on e.draft_id = do.draft_id and e.oper_id = do.oper_id
                        join projects_durations dur1 on dur1.duration_id = do.duration_wk_id
                        join projects_durations dur_h on dur_h.duration_id = 2
                join projects_durations dur2 on dur2.duration_id = o.duration_wk_id
            where o.d_to_plan between @period_from and @period_to
            group by o.place_id, o.mfr_doc_id, c.product_id, e.post_id
            having sum(c.q_brutto_product * do.duration_wk * dur1.factor / dur_h.factor) > 0.1;
...
GO
-- exec mfr_reps_jobs_persons 1000, @folder_id = -1, @context = 'contents'

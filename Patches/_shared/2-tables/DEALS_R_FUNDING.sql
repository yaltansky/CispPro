...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[DEALS_R_FUNDING]') AND type in (N'U'))
BEGIN
CREATE TABLE [DEALS_R_FUNDING](
...
IF NOT EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[DEALS_R_FUNDING]') AND name = N'IX_DEAL')
CREATE NONCLUSTERED INDEX [IX_DEAL] ON [DEALS_R_FUNDING]
(
	[DEAL_ID] ASC
)
GO
